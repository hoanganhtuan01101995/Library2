...
public class PhoneSignupFragmentPhase2 extends DialogFragment implements OnClickListener {
    public static final String CLASS_NAME = PhoneSignupFragmentPhase2.class.getName();
    private static final String TAG = PhoneSignupFragmentPhase2.class.getSimpleName();
    ...
    public static PhoneSignupFragmentPhase2 newInstance(Context context) {
        PhoneSignupFragmentPhase2 fragment = new PhoneSignupFragmentPhase2();
        mContext = context;
        return fragment;
    }

    public PhoneSignupFragmentPhase2() {
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.fragment_signup_phone_num_p2, container, false);
...
//        mEmailCheckMessageView.setText("");
        phoneNumber = mIdInput.getText().toString();
        showProgress();
...
//        FrontEndLoader.getInstance().checkID();
        ...
        FrontEndLoader.getInstance().createAccount(email, password, code, new WindmillCallback() {
            @Override
            public void onSuccess(Object obj) {
                hideProgress();
    ...
    private void requestAuthCode() {
        ...
        FrontEndLoader.getInstance().requestAuthenticationCode(phoneNumber, new WindmillCallback() {
            @Override
            public void onSuccess(Object obj) {
                hideProgress();
            ...
            switch (msg.what) {
                case RUN:
                    ...
                    if (cnt > 0) {
                        ...
                        authInvalideCnt.setText(String.format(mContext.getString(R.string.create_account_auth_cnt_title), changeTime(cnt--)));
//                        auchCnt.setText(changeTime(cnt--));
                        ...
                        authCntLayout.setVisibility(View.VISIBLE);
                        mSmsInput.setEnabled(false);
                        authInvalideCnt.setText(mContext.getString(R.string.create_account_auth_cnt_over));
//                        auchCnt.setText("");
    ...
    private class SignUpTextWatcher implements TextWatcher {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            if (i == R.id.id_input) {
                ...
                mVerifyButton.setEnabled(mIdInput.length() > 0 && !mIdInput.getText().toString().equals(phoneNumber));
...
//            mPasswordConfirmInput.setEnabled(mPasswordInput.length() > 0);
            ...
            if (pwString.length() != 0 && pwString.length() == pwConfirmString.length()) {
                if (pwString.equals(pwConfirmString)) {
                    mPasswordCheckMessageView.setText(R.string.msgSignupRightConfirmPassword);
                    mPasswordCheckMessageView.setTextColor(getResources().getColor(R.color.green_success));
                    mPasswordCheckMessageView.setTextColor(0xFF00C4BF);
                    mImvCheckMessage.setVisibility(View.VISIBLE);
                } else {
                    mPasswordCheckMessageView.setText(R.string.myaccount_wrongpassword);
                    mPasswordCheckMessageView.setTextColor(getResources().getColor(R.color.red_error));
...
//                    mPasswordInput.length() * mPasswordConfirmInput.length() != 0);
            ...
            if (pwString != null && pwString.length() > 0) {
                mPasswordConfirmInput.setFocusableInTouchMode(true);
