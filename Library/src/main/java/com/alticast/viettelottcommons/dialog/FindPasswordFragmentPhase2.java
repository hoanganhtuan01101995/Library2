...
public class FindPasswordFragmentPhase2 extends DialogFragment implements OnClickListener {
    public static final String CLASS_NAME = FindPasswordFragmentPhase2.class.getName();
    private static final String TAG = FindPasswordFragmentPhase2.class.getSimpleName();
    ...
    public static FindPasswordFragmentPhase2 newInstance(Context context) {
        FindPasswordFragmentPhase2 fragment = new FindPasswordFragmentPhase2();
        return fragment;
    }

    public FindPasswordFragmentPhase2() {
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.fragment_find_password_p2, container, false);
...
//        auchCnt = (TextView) v.findViewById(R.id.auchCnt);
        ...
        mPasswordConfirmInput.setFocusableInTouchMode(false);
        mCreateButton = (Button) v.findViewById(R.id.create_button);
        mCreateButton.setText(R.string.create_new_password);
    ...
    private void setFragmentEnable(int number) {
        ...
        switch (number) {
            case PAGE_ONE:
                ...
                globalActivity.showKeyBoard(mIdInput);
                break;
            case PAGE_TWO:
                ...
                layout_number_three_normal.setVisibility(View.VISIBLE);
                initSceneNumberTwo();
                globalActivity.showKeyBoard(mSmsInput);
                break;
            case PAGE_THREE:
                layout_number_one_focus.setVisibility(View.GONE);
                layout_number_two_focus.setVisibility(View.GONE);
                ...
                globalActivity.showKeyBoard(mPasswordInput);
                break;
        }
    }
...
//            mEmailCheckMessageView.setVisibility(View.INVISIBLE);
            mImvCheckMessage.setVisibility(View.GONE);
        }
    }
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.btn_back) {
            ...
            createNewPassword();
        } else if (i == R.id.layout_one_normal && getCurrentSignUpFragment() != PAGE_ONE) {
            setFragmentEnable(PAGE_ONE);
        } else if (i == R.id.layout_two_normal && getCurrentSignUpFragment() == PAGE_TWO) {
            if (cntHandler != null && cntHandler.getType() != CntHandler.OVER) {
                setFragmentEnable(PAGE_TWO);
            }
        } else if (i == R.id.sms_button_confirm && isExistOTPString()) {
            showProgress();
            FrontEndLoader.getInstance().verifyAuthenticationCode(phoneNumber, otp, new WindmillCallback() {
                @Override
                public void onSuccess(Object obj) {
                    VerifyCodeRes verifyCodeRes = (VerifyCodeRes) obj;
                    authenCode = verifyCodeRes.getCode();
    ...
    public void showExitDialog() {
        String title = getString(R.string.pop_quit_create_password_title);
    ...
    private void verifyPhoneNumber() {
        //HuyLV [06/28/2016]############################
        phoneNumber = mIdInput.getText().toString().trim();
        //##############################################
        showProgress();
        FrontEndLoader.getInstance().checkID(phoneNumber, new WindmillCallback() {
            @Override
            public void onSuccess(Object obj) {
...
//                    mVerifyCodeEdt.setEnabled(true);
    ...
    private void createNewPassword() {
        ...
        //HuyLV [06/28/2016]############################
        final String email = mIdInput.getText().toString().trim();
        String code = authenCode;
        String password = mPasswordConfirmInput.getText().toString();
        //##############################################
...
//                imm.hideSoftInputFromWindow(mVerifyCodeEdt.getWindowToken(), 0);
                ...
                App.getToast(getActivity(), getString(R.string.forgot_pwd_step3_msg), getString(R.string.create_account_noti, email), false).show();
    ...
    private void processChangeSuccess() {
        if(HandheldAuthorization.getInstance().isLogIn()) {
            ...
            GlobalActivity globalActivity = (GlobalActivity) getActivity();
            globalActivity.removeProfileFragment();

            Intent intent = new Intent(GlobalKey.MainActivityKey.REFRESH_DATA);
            LocalBroadcastManager.getInstance(globalActivity).sendBroadcast(intent);

            globalActivity.showLoginPairingDialog(false);
        }
            ...
            switch (msg.what) {
                case RUN:
                    ...
                    if (cnt > 0) {
                        ...
                        authInvalideCnt.setText(String.format(getString(R.string.create_account_auth_cnt_title), changeTime(cnt--)));
...
//            mPasswordConfirmInput.setEnabled(mPasswordInput.length() > 0);
            ...
            mPasswordConfirmInput.setFocusableInTouchMode(pwString.length() > 0);
            if (pwString.length() == pwConfirmString.length() && pwString.length() > 0) {
                if (pwString.equals(pwConfirmString)) {
                    mPasswordCheckMessageView.setText(R.string.msgSignupRightConfirmPassword);
                    mPasswordCheckMessageView.setVisibility(View.VISIBLE);
